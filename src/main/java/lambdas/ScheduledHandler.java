...
public class ScheduledHandler {
    ...
    public APIGatewayProxyResponseEvent handleRequest() throws IOException {
        ...
        if (conn.getResponseCode() != 200) {
            throw new IllegalArgumentException("Failed : HTTP error code : " + conn.getResponseCode());
        }
        ...
        ExchangeRepository.getInstance().batchSave(exchangeRateBatch);
    ...
    public List<ExchangeRateDTO> getExchangeRateRecords(ExchangeRate parsed) {
		...
		logger.info("Successfully parsed exchange rate records");
